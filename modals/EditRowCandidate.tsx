...
type AgreementValue = 'Y' | 'N';

const EditRowCandidate: React.FC<EditRowCandidateProps> = ({ isOpen, onRequestClose, rowData, onSave }) => {
  const [formData, setFormData] = useState<Candidate & { agreement: AgreementValue }>({
    candidateid: rowData?.candidateid,
    name: rowData?.name,
    enrolleddate: rowData?.enrolleddate,
    email: rowData?.email,
    ...
    workexperience: rowData?.workexperience,
    ssn: rowData?.ssn,
    agreement: rowData?.agreement ? 'Y' : 'N',
    promissory: rowData?.promissory,
    driverslicense: rowData?.driverslicense === true ? 'Y' : 'N',
    workpermit: rowData?.workpermit === true ? 'Y' : 'N',
    wpexpirationdate: rowData?.wpexpirationdate,
  ...
  useEffect(() => {
    if (rowData) {
      setFormData({
        ...rowData,
        agreement: rowData.agreement ? 'Y' : 'N',
        driverslicense: rowData.driverslicense ? 'Y' : 'N',
        workpermit: rowData.workpermit ? 'Y' : 'N',
      });
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const dataToSend = {
        ...formData,
        agreement: formData.agreement === 'Y',
        driverslicense: formData.driverslicense === 'Y',
        workpermit: formData.workpermit === 'Y',
      };
      await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/candidates/update/${formData.candidateid}`, dataToSend, {
        headers: { AuthToken: localStorage.getItem('token') },
      });
      onSave();
      onRequestClose();
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {Object.keys(formData).map((key) => (
          <div key={key} className="modal-field">
            ...
            {(key === 'agreement' || key === 'driverslicense' || key === 'workpermit') && (
              <select
                id={key}
                name={key}
                value={formData[key as keyof typeof formData]}
