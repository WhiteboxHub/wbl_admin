...
//   const handleSaveRow = async (newRow: Lead) => {
//     try {
//       const token = localStorage.getItem('token');
//       const url = `${API_URL}/api/leads/insert`;
...
//   const handleEditUpdatedRow = async (newRow: Lead) => {
//     try {
//       await axios.put(`${API_URL}/api/leads/update/${newRow.leadid}`, newRow, {
//         headers: { AuthToken: localStorage.getItem('token') },
//       });
...
//   }, [currentPage, paginationPageSize, API_URL]);
...
//       window.removeEventListener('keydown', handleKeyDown);
//     };
//   }, [currentPage, paginationPageSize, router, fetchData, resetSearch]);
...
//           columnDefs={columnDefs}
//           pagination={true}
//           rowSelection="single"  // Directly specify rowSelection here
//           onRowClicked={(event) => event.node.setSelected(true)}
//           getRowStyle={(params) => params.node.isSelected() ? { backgroundColor: 'lightyellow' } : null}
//         />
//       </div>
//       {isSearchVisible && (
//         <div className="search-container">
...
const Leads = () => {
  ...
  const handleEditUpdatedRow = async (newRow: Lead): Promise<void> => {
    try {
      await axios.put(`${API_URL}/api/leads/update/${newRow.leadid}`, newRow, {
        headers: { AuthToken: localStorage.getItem('token') },
      });
    ...
    <div className="custom-grid-container">
      ...
      <div className="ag-theme-alpine">
        <AgGridReact
          ...
          rowSelection="single"  // Directly specify rowSelection here
          onRowClicked={(event) => event.node.setSelected(true)}
          getRowStyle={(params) => params.node.isSelected() ? { backgroundColor: 'lightyellow' } : undefined}
        />
      </div>
